...
if(isset($_POST['submit_val-1'])) {
    ...
    $addPupdateid = $_REQUEST['update-id'];
...
///echo "<script>alert('$finaltotal')</script>";
    ...
    if ($insert_id == 'insert') {
        ...
        $quofferidval1 = $_REQUEST['offeridval'];
...
VALUES ('$addProName1[$key]','$unitprice[$key]','$quantity[$key]','$total[$key]','$finaltotal[$key]','$finaltotal[$key]','$sgtdiscTotal[$key]','$dissgst[$key]','$sgstTotal[$key]','$cgst[$key]','$cgsttotal[$key]','$billNo','$shopidval','$fid','$cofferidval[$key]','$proId','Q','$qHSNNo1[$key]')";
            $record = mysqli_query($conn, $query);
        }


    }
    if ($addPupdateid=='update'){
    ...
    if($record) {
        ...
        echo "<script>window.location.href='taxQuatationGeneratedBill.php?custid=$str&proId=$str1;'</script>";
...
<script>
    ...
    function accordingQtyCalulate(str) {
        ...
        if(amtOffer22 > 0 ) {
            ...
            document.getElementById('finaltotal'+ res).value = sutota1;
