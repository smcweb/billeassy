...
<div id="wrapper">
    ...
    <div class="content_bottom">
     <div class="col-md-12 span_3">
		...
		$max=$max+1;
			?>
       		 <input type="hidden" class="form-control1" id="expid" name="expid" value="<?php if(isset($max)){echo $max;}?>">
      	<!--</div>-->
...
<?php $tadate=date('Y-m-d'); ?>		<!---- autogenerated  -------->
	  ...
	  <div class="form-group has-warning">
        ...
        <input type="text" class="form-control1"  id="exp_amount" name="exp_amount"  pattern="[1-9]\d*"  required>
      </div>
	   <div class="form-group has-warning">
        <label style="color:black" style="width: 90%;" for="inputWarning1">Description</label>
        <input type="text" class="form-control1" id="exp_desc" name="exp_desc" >
      </div>
	  <div class="form-group has-warning">
        ...
        <input type="text" class="form-control1" id="exp_by" name="exp_by" required>
	 	 	 ...
	 	 	 <input type="submit" class="btn btn-success" name="Submit" id="dadd" value="Add">
								...
								if(isset($_POST['txt_search']))
									{
                                        ...
                                        $search_result="";
                                        $query = "SELECT SUM(`exp_amount`) exp_amount,exp_date,exp_name,exp_by,expid FROM tblexpences WHERE `exp_name` LIKE '%$search%' || `exp_by` LIKE '%$search%' AND shopid='$shopid' GROUP BY `exp_by`";
									...
									elseif(isset($_POST['date_search']))
									{
                                        ...
                                        $search_result="";
                                        $query = "SELECT SUM(`exp_amount`) exp_amount,exp_date,exp_name,exp_by,expid FROM tblexpences WHERE `exp_date` LIKE '$search' AND shopid='$shopid' GROUP BY `exp_by`";
                                    $search_result = mysqli_query($conn, $query); 
                                    ...
                                    {
                                        $search_result="";
                                        $query = "SELECT SUM(`exp_amount`) exp_amount,exp_date,exp_name,exp_by,expid  FROM tblexpences WHERE shopid='$shopid' GROUP BY `exp_by`";
								  ...
								  while( $row = mysqli_fetch_array( $search_result ))
								  {
									...
									echo"<td>".$row['exp_amount']."</td>";
									  ...
									  <td>
                                          ...
                                          <a  href="viweAllExpencess.php?exp_by=<?php echo $row['exp_by'] ?>"><i class="fa fa-eye iconsize" aria-hidden="true"></i></a>
</td>
				   <?php
