...
VALUES('$addProName1[$key]','$unitprice[$key]','$quantity[$key]','$total[$key]','$finaltotal[$key]','$discount_price[$key]','$billNo','$fid','T','$proId','$qHSNNo1[$key]','$shopidval','$quofferidval1[$key]')
";
            $record = mysqli_query($conn, $query);
    ...
    foreach ($toitemid as $key => $valuein) {
        ...
        while ($row = mysqli_fetch_array($recordSelect)) {
            ...
            $qHSNN11 = $_REQUEST['HSNNo'];
            $total = $_REQUEST['total'];
            $discount_price = $_REQUEST['discount_price'];

            $finaltotal = $_REQUEST['finaltotal'];
            $offerPercent = $_REQUEST['offerPercent'];
            if ($quantity[$key] != $nfypnameitemid) {
                $query = "UPDATE `tblitem` SET `pname`='$addProName[$key]',`unitprice`='$unitprice[$key]',`quantity`='$quantity[$key]',`total`='$total[$key]',
`taxtotal`='$finaltotal[$key]',`discount_price`='$discount_price[$key]',`billno`='$billNo',`fid`='$fid',`statusQT`='T',hsncode='$qHSNN11[$key]' WHERE `itemid`='$toitemid[$key]' and `tid` ='$proId' AND shopid='$shopidval'";
                $record = mysqli_query($conn, $query);
    ...
    if($record) {
        ...
        echo "<script>window.location.href='billInvoiceGenerated.php?custid=$str&proId=$str1;'</script>";
    ...
    $(document).ready(function () {
        ...
        function fillDataInTable(searchItem) {
            ...
            for (var i = 0; i < searchItem.length; i++) {
                ...
                foodTable += "<td><i id='"+searchItem[i].itemid+"' style='margin: 0px 35px;color: red;font-size: 20px' title='Delete' class='fa fa-trash-o admin_rowControls admindeleteItem'></i></td> </tr>";
            ...
            foodTable += '<input  type="hidden"  name="update"  id="update"    style="float: left;width: 75px;" >';
