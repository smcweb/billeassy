...
if(isset($_POST['submit_val'])) {
    ...
    $query="INSERT INTO `tbltransaction`( `billno`, `tdate`, `btotal`, atotal,`discount`, `remaining`, `shopid`, `fid`, `modeOfPayment`,custid,remark,statusQT,paid,inwords) VALUES ('$billNo','$todate','$txt_samt1','$txt_gamt','$txt_discount','$remaining','$shopidval','$fid','$modeOfPayment','$cid','$onlineCashCheque','T','$txt_paiding','$gwords')";
    $record=mysqli_query($conn,$query);
...
VALUES ('$addProName[$key]','$unitPrice[$key]','$qty[$key]','$totalAmt[$key]','$taxPer[$key]','$finalTotalAmt[$key]','$gstName[$key]','$billNo','$shopidval','$fid','$txt_offerval[$key]','$last_row','$txt_discountPrice[$key]','T')";
        $record=mysqli_query($conn,$query);
    ...
    if($record) {
        ...
        echo "<script>window.location.href='billInvoiceGenerated.php?custid=$cdid&proId=$pid;'</script>";
        ...
        <form action="" method="post">
            <div id="company" style='margin: 0px -38px'>
                <div>
                    ...
                    <span><span>Moblie No.&nbsp;&nbsp;&nbsp;&nbsp;<input type="number" name="number" onKeyPress="if(this.value.length==10) return false;"  id="number" style='' onblur="getDataNumberWise()" value="1234567890"> </span></span>
                </div><br/>
                <div>
