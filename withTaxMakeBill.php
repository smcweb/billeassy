...
$txt_modeOfPayment=$_REQUEST['modeOfPayment'];
$txt_cashid=$_REQUEST['cashid'];
/*$txt_paiding=$_REQUEST['txt_paiding'];
$txt_remeingBal=$_REQUEST['remeingBal'];*/
    $txt_paiding=0;
    $txt_remeingBal=0;
    $txt_hddHsncode=$_REQUEST['addHsncode'];
    $txt_addProName=$_REQUEST['addProName1'];
    $txt_MRP=$_REQUEST['unitPrice'];
    $txt_withMRP=$_REQUEST['unitPricewith'];
    $txt_qty=$_REQUEST['qty'];
    $txt_totalAmt=$_REQUEST['totalAmt'];
    $txt_cgst=$_REQUEST['cgst'];
    $txt_cgsttotal=$_REQUEST['cgsttotal'];
    $txt_sgst=$_REQUEST['sgst'];
    $txt_sgstTotal=$_REQUEST['sgstTotal'];
    $txt_sgstTotal=$_REQUEST['sgstTotal'];
    $txt_totlgstcgst=$_REQUEST['totlgstcgst'];
    ...
    if($custnumber!=$numphonenumber){
        $query="INSERT INTO `phonebook`( `name`, `phonenumber`, `email`,gst, `address`, `pgroup`, `shopid`,venders,statesCode,states) VALUES ('$custname','$custnumber','$custEmail','$custGSTNo','$custAddr','$fid','$shopidBn','customer','$custStCode','$custState')";
    ...
    $query = "UPDATE `phonebook` SET `name`='$custname',`phonenumber`='$custnumber',`email`='$custEmail',`address`='$custAddr',`states`='$custState',`gst`='$custGSTNo',`statesCode`='$custStCode',`balanceAmt`='0' WHERE `cid`='$cid'";
    $record = mysqli_query($conn, $query);
    $nfyear1=substr("$nfyear",27);
    $nfyear21=substr("$nfyear",10,3);
    $fidd3=$nfyear21.'-'. $nfyear1;
    $billNo=$fidd3.'/'.'00'.$nid2;
    $query="INSERT INTO `tbltransaction`( `custid`, `billno`, `tdate`, `btotal`, `discount`, `atotal`, `paid`, `inwords`, `stateCode`, `remaining`, `shopid`, `fid`, `modeOfPayment`, `remark`, `statusQT`, `typeTrnj`)
 VALUES ('$cid','$billNo','$invoiceDate','$txt_samt1','$txt_disc','$txt_gamt1','$txt_paiding','$txt_words','$custStCode','$txt_remeingBal','$shopidBn','$fid','$txt_modeOfPayment','$txt_cashid','T','withTax')";
    $record=mysqli_query($conn,$query);
    $last_row =  mysqli_insert_id($conn);
foreach ($txt_qty as $key => $valuein) {
    $query = "INSERT INTO `tblitem`( `pname`, `unitprice`,with_mrp_tax_price, `quantity`, `total`, `finaltotal`,`taxtotal`, `discount_price`, `taxname`, `taxAmtSgst`, `taxNameCgst`, `taxAmtCgst`, `billno`, `shopid`, `fid`, `oid`, `tid`, `statusQT`, `hsncode`) 
VALUES ('$txt_addProName[$key]','$txt_MRP[$key]','$txt_withMRP[$key]','$txt_qty[$key]','$txt_totalAmt[$key]','$txt_totlgstcgst[$key]','$txt_finalTotalAmt[$key]','$txt_discountPrice[$key]','$txt_sgst[$key]','$txt_sgstTotal[$key]','$txt_cgst[$key]','$txt_cgsttotal[$key]','$billNo','$shopidBn','$fid','$txt_offeridval[$key]','$last_row','T','$txt_hddHsncode[$key]')";
    $record = mysqli_query($conn, $query);
    $query="UPDATE `tblproduct` SET `quantity`=`quantity`-$txt_qty[$key] WHERE `pname`='$txt_addProName[$key]'";
    $record=mysqli_query($conn,$query);
}
    if($record) {
    ...
    setTimeout(function() {
        swal({
            ...
            window.location = 'taxInvoice.php?custid=$cdid&proId=$pid';
        });
    }, 500);
</script>";
        /*echo "<script>alert('Wait For Print');</script>";
        echo "<script>window.location.href='taxInvoice.php?custid=$cdid&proId=$pid;'</script>";*/
...
#openSidebarMenu:checked ~ .sidebarIconToggle > .spinner.bottom {
                    ...
                    <table class="table table-condensed">
                        ...
                        <tr>
                            ...
                            <th><input type="text" name="stCode" id="stCode" style="" required=""
                                       ...
                                       <th><input type="text" name="pincode" placeholder="pincode" class="pin" ></th>
                                       <tr>
                                        <th><input type="text" name="age" class="age" placeholder="age"></th>
                                        <th><input type="text" name="gender" class="gender" placeholder="gender"></th>
                                        <th><input type="text" name="adhaarnumber" class="adhaar" placeholder="Adhaar number"></th>
    ...
    padding: 0px 0px 13px;" placeholder="description">
                        ...
                        <tr id="cheque-1">
                            <td></td>
                            <td><input type="text" name="cashid" id="onlineCashCheque" style="
        width: 240px;
    float: left;
    ...
    padding: 0px 0px 13px;" placeholder="finance"></td>
                ...
                <div class="col-lg-4" style="margin: 0px 95px;">
                    <table class="table table-bordered">
                        <thead>
                        <tr>
                        ...
                        <tr>
                            ...
                            <th>
                                <input type="text" name="disc" placeholder="In Rupees" onblur="discountCalulate()" id="discount-tot" value="0" style="float: left; z-index:111111111111111111">
                            </th>
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (pnameCount == same) {
                    ...
                    data += "<td><input type='text' id='prohsn" + FieldCount + "' name='addHsncode[]' class='form-control-2'/></td> <td><input type='text' id='proName" + FieldCount + "' name='addProName1[]' style='width: 124px;' /></td><td><input type='text' id='unitp" + FieldCount + "' name='unitPrice[]' class='form-control-2' onblur='mrpCalculatePrice(this.id)'/></td><td><input type='text' id='unitpwith" + FieldCount + "' name='unitPricewith[]' class='form-control-2' onblur='mrpWithTaxCalculatePrice(this.id)'/></td></td><td><input type='text' id='qtyUp" + FieldCount + "' name='qty[]' class='form-control-2' onblur='qtyCalculate(this.id)'/></td><td><input type='text' id='nod-Total" + FieldCount + "' name='totalAmt[]' class='form-control-2' readonly/></td><td><input type='text' id='cgst" + FieldCount + "' name='cgst[]' class='form-control-2' onblur='calculateCGSTSGST(this.id)'/></td><td><input type='text' id='cgsttotal" + FieldCount + "' name='cgsttotal[]' class='form-control-2' readonly/></td><td><input type='text' id='sgst" + FieldCount + "' name='sgst[]' class='form-control-2' onblur='calculateCGSTSGST(this.id)'/><td><input type='text' id='sgstTotal" + FieldCount + "' name='sgstTotal[]' class='form-control-2' readonly/></td><td><input type='text' id='totlgstcgst" + FieldCount + "' name='totlgstcgst[]' class='form-control-2' readonly/></td><td><input type='text' id='discountPrice" + FieldCount + "' name='discountPrice[]' class='form-control-2' readonly/></td><td><input type='text' id='offerName" + FieldCount + "' name='offerid[]' class='form-control-2' onblur='calculateOffer(this.id)'/></td><td><input type='text' id='total" + FieldCount + "' name='finalTotalAmt[]' class='form-control-2' readonly/></td><input type='hidden' id='offerval" + FieldCount + "' class='form-control'  name='offeridval[]' ><td><a href='javascript:void(0)'  class='remCF' id='" + FieldCount + "' ><i class='fa fa-trash-o' style='font-size: 26px;color: red;'></i></a></td></tr>";
