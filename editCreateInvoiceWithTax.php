...
///echo "<script>alert('$finaltotal')</script>";
   ...
   // echo "<script>alert('$toitemid')</script>";
    ...
    $query="UPDATE `tbltransaction` SET `tdate`='$todate',`btotal`='$txt_samt1',`discount`='$txt_discount',`atotal`='$txt_gamt',`paid`='$txt_paiding',`inwords`='$cwords',`stateCode`='$statesCode',`remaining`='$remaining',`fid`='$fid',`modeOfPayment`='$onlineCashCheque',`statusQT`='T' WHERE tid ='$proId' AND shopid='$shopidval' ";
    $record=mysqli_query($conn,$query);
...
if($gsnuupdate == 'update') {
    ...
    $taxNameCgst=$_REQUEST['taxNameCgst'];
    $taxAmtCgst=$_REQUEST['taxAmtCgst'];
    $taxnameSgst=$_REQUEST['taxnameSgst'];
    $taxAmtSgst=$_REQUEST['taxAmtSgst'];
    $ttaxtotal=$_REQUEST['taxtotal'];
    ...
    $finaltotalAmt=$_REQUEST['finaltotal'];
    foreach ($toitemid as $key => $valuein) {
        $querySelect = "SELECT quantity FROM `tblitem` WHERE `tid` ='$proId' and `itemid`='$toitemid[$key]'  ";
        $recordSelect = mysqli_query($conn, $querySelect);

        while ($row = mysqli_fetch_array($recordSelect)) {
            $nfypnameitemid = $row['quantity'];

            $query = "UPDATE `tblitem` SET `pname`='$addProName[$key]',`unitprice`='$unitprice[$key]',`quantity`='$quantity[$key]',`hsncode`='$HSNNo[$key]',`taxNameCgst`='$taxNameCgst[$key]',taxAmtCgst='$taxAmtCgst[$key]',taxname='$taxnameSgst[$key]',taxAmtSgst='$taxAmtSgst[$key]',taxtotal='$ttaxtotal[$key]',`total`='$total[$key]',`finaltotal`='$finaltotalAmt[$key]',`discount_price`='$txt_discountPrice[$key]',`billno`='$billNo',`fid`='$fid',`statusQT`='T' WHERE `itemid`='$toitemid[$key]' AND shopid='$shopidval'";
            $record = mysqli_query($conn, $query);
            $query = "UPDATE `tblproduct` SET `quantity`=`quantity`-$qty[$key] WHERE `pname`='$addProName[$key]'";
            $record = mysqli_query($conn, $query);
            if ($nfypnameitemid < $quantity[$key]) {
                $finalqty = $quantity[$key] - $nfypnameitemid;
                $query = "UPDATE `tblproduct` SET `quantity`=`quantity`-'$finalqty' WHERE `pname`='$addProName[$key]'";
                $record = mysqli_query($conn, $query);
            }
            if ($nfypnameitemid > $quantity[$key]) {
                $finalqty = $nfypnameitemid - $quantity[$key];
                $query = "UPDATE `tblproduct` SET `quantity`=`quantity`+'$finalqty' WHERE `pname`='$addProName[$key]'";
                $record = mysqli_query($conn, $query);
            }
        }
    ...
    if ($insert_id == 'insert') {
        $addProName1 = $_REQUEST['addProName1'];
        $qHSNNo1 = $_REQUEST['addHsncode'];
        $quantity = $_REQUEST['qty'];
        $quofferidval1 = $_REQUEST['offeridval1'];
        $unitprice = $_REQUEST['unitPrice'];

        $total = $_REQUEST['stotal'];

        $cgst = $_REQUEST['cgst'];
        $cofferidval = $_REQUEST['offeridval'];
        $cgsttotal = $_REQUEST['cgsttotal'];

        $dissgst= $_REQUEST['sgst'];
        $sgstTotal= $_REQUEST['sgstTotal'];
        $sgtotlgstcgst= $_REQUEST['totlgstcgst'];
        $sgtofferAmt= $_REQUEST['offerAmt'];
        $sgtdiscTotal= $_REQUEST['discTotal'];

        $finaltotal = $_REQUEST['ftotalval'];
        foreach ($quantity as $key => $valuein) {

            $query = "INSERT INTO `tblitem`( `pname`, `unitprice`, `quantity`, `total`, `finaltotal`,  `taxtotal`, `discount_price`, `taxname`, `taxAmtSgst`, `taxNameCgst`, `taxAmtCgst`, `billno`, `shopid`, `fid`, `oid`, `tid`, `statusQT`, `hsncode`) 
VALUES ('$addProName1[$key]','$unitprice[$key]','$quantity[$key]','$total[$key]','$finaltotal[$key]','$finaltotal[$key]','$sgtdiscTotal[$key]','$dissgst[$key]','$sgstTotal[$key]','$cgst[$key]','$cgsttotal[$key]','$billNo','$shopidval','$fid','$cofferidval[$key]','$proId','T','$qHSNNo1[$key]')";

            $record = mysqli_query($conn, $query);
            $query = "UPDATE `tblproduct` SET `quantity`=`quantity`-'$quantity[$key]' WHERE `pname`='$addProName1[$key]'";
            $record = mysqli_query($conn, $query);
        }
    ...
    if($record) {
        ...
        echo "<script>window.location.href='billInvoiceGeneratedWithtax.php?custid=$str&proId=$str1;'</script>";
...
<script>
    ...
    $(document).ready(function () {
        ...
        function fillDataInTable(searchItem) {
            ...
            foodTable += "<tr>"+
                "<th style='text-align: left;    padding: 5px 2px;'>Item Name</th><th style='text-align: left;    padding: 5px 2px;'>HSN No. </th><th style='text-align: left;    padding: 5px 2px;'>Qty</th><th style='text-align: left;    padding: 5px 2px;'>Unit Price </th></th>" +
                "<th style='text-align: left;    padding: 5px 24px;'> Total </th><th style='text-align: left;    padding: 5px 12px;'> C-GST </th><th style='text-align: left;    padding: 5px 21px;'> Amt </th><th style='text-align: left;    padding: 5px 18px;'> S-GST </th><th style='text-align: left;    padding: 5px 26px;'> Amt </th><th style='text-align: left;padding: 5px 17px;'> Net Amt </th><th style='text-align: left;    padding: 5px 22px;'> Offer %</th>" +
                "<th style='text-align: left;    padding: 5px 2px;'> Discount Price </th><th style='text-align: left;    padding: 5px 25px;'> Final Total </th><th style='text-align: left;    padding: 5px 23px;'>Modify </th></tr>";

            for (var i = 0; i < searchItem.length; i++) {
                foodTable += "<tr class='highlight-row' id='remove-id-if-user"+searchItem[i].itemid+"' >";
                foodTable += "<td style='padding: 2px 19px;'>" +'<input  type="text"  name="productname1[]"  id="pname'+searchItem[i].itemid+'" class="form-control"  style=" float: left; margin: 1px -19px ;width: 112px;"  value="' + searchItem[i].pname + '" readonly >' + "</td>";
                foodTable += "<td style='padding: 5px 6px; padding: 5px 4px;'>" +'<input  type="text"  name="HSNNo[]"  id="HSNNo'+searchItem[i].itemid+'" class="form-control" style=" float: left;width: 33px;" value="' + searchItem[i].hsncode + '">' + "</td>";
                foodTable += "<td style='padding: 5px 4px;'>" +'<input  type="text"  name="quantity[]" onblur="accordingQtyCalulate(this.id)"  id="quantity'+searchItem[i].itemid+'"  value="' + searchItem[i].quantity + '"   style="float: left;width:20px;"  >' + "</td>";
                foodTable += "<td style='padding: 5px 4px;'>" +'<input  type="text"  name="unitprice-1[]"  onblur="accordingQtyCalulate(this.id)" id="unitpric'+searchItem[i].itemid+'"  value="' + searchItem[i].unitprice + '"  style="float: left;width: 50px;" >' + "</td>";
                foodTable += "<td     padding: 5px 15px;>" +'<input  type="text"  name="total[]"  id="total'+searchItem[i].itemid+'"  value="' + searchItem[i].total + '"  style="float: left;width: 41px;" readonly>' + "</td>";
                foodTable += "<td>" +'<input  type="text"  name="taxNameCgst[]"  id="taxNameCgst'+searchItem[i].itemid+'" onblur="accordingGSTCalulate(this.id)"   value="' + searchItem[i].taxNameCgst + '"  style="float: left;width: 21px;" >' + "</td>";
                foodTable += "<td>" +'<input  type="text"  name="taxAmtCgst[]"  id="taxAmtCgst'+searchItem[i].itemid+'"  value="' + searchItem[i].taxAmtCgst + '"  style="float: left;width: 28px;" readonly>' + "</td>";
                foodTable += "<td>" +'<input  type="text"  name="taxnameSgst[]"  id="taxnameSgst'+searchItem[i].itemid+'"  value="' + searchItem[i].taxname + '"  onblur="accordingGSTCalulate(this.id)" style="float: left;width: 41px;" >' + "</td>";
                foodTable += "<td>" +'<input  type="text"  name="taxAmtSgst[]"  id="taxAmtSgst'+searchItem[i].itemid+'"  value="' + searchItem[i].taxAmtSgst + '"  style="float: left;width: 51px;" readonly>' + "</td>";
                foodTable += "<td>" +'<input  type="text"  name="taxtotal[]"  id="taxtotal'+searchItem[i].itemid+'"  value="' + searchItem[i].taxtotal + '"  style="float: left;width: 61px;" readonly>' + "</td>";
                foodTable += "<td>" +'<input  type="text"  name="offerPercent[]"  id="offerPercent'+searchItem[i].itemid+'"  value="' + searchItem[i].offerPercent + '"  style="float: left;width: 37px;" >' + "</td>";
                foodTable += "<td>" +'<input  type="text"  name="discount_price[]"  id="discount_price'+searchItem[i].itemid+'"  value="' + searchItem[i].discount_price + '"  style="float: left;width: 33px;" >' + "</td>";
                ...
                foodTable += "<td style='padding: 12px 0px;font-size: 20px;color: red;'><i id='"+searchItem[i].itemid+"' style='margin: 0px 40px;' title='Delete' class='fa fa-trash-o admin_rowControls admindeleteItem'></i></td> </tr>";
            ...
            foodTable +='<input type="hidden" class="form-control" name="update-id" id="update-id" >';
          ...
          $.ajax({
              ...
              success: function (response) {
                  ...
                  if(pnameCount==same){
                      ...
                      $(wrapper).append('<div id="remove-id-if-user'+FieldCount+'"><input type="text" id="pname'+FieldCount+'"  class="form-control" style="margin-top: 8px;width: 10%;"size="15%" name="addProName1[]"/>&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" id="HSNNo'+FieldCount+'"  class="form-control" style="margin-top: 8px;margin-left: -1%;width: 4%;"size="15%" name="addHsncode[]"/>&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" id="quantity'+FieldCount+'" class="form-control" style="margin-top: 8px;width: 2%;    margin-left: -6px" onblur="accordingQtyCalulate(this.id)" name="qty[]"  data-id='+FieldCount+' required>&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" id="unitpric'+FieldCount+'"  class="form-control" style="margin-top: 8px;width:5%;margin-left: -7px;" name="unitPrice[]"/>&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" id="total'+FieldCount+'" class="form-control" style="margin-top: 8px;width: 5%;margin-left: 4px;"  name="stotal[]" readonly/>&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" id="taxNameCgst'+FieldCount+'" class="form-control" style="margin-top: 8px;width: 2%;margin-left: 23px;" onblur="accordingGSTCalulate(this.id)" name="cgst[]" >&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" id="taxAmtCgst'+FieldCount+'" class="form-control" style="margin-top: 8px;width: 3%;    margin-left: 24px;"  name="cgsttotal[]" readonly>&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" id="taxnameSgst'+FieldCount+'" class="form-control" style="margin-top: 8px;width: 4%;    margin-left: 24px;"  name="sgst[]" onblur="accordingGSTCalulate(this.id)" >&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" id="taxAmtSgst'+FieldCount+'" class="form-control" style="margin-top: 8px;width: 5%;    margin-left: 24px;"  name="sgstTotal[]" readonly />&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" id="taxtotal'+FieldCount+'"  class="form-control" style="margin-top: 8px;width: 6%;    margin-left: 19px;"   name="totlgstcgst[]">&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" id="offerPercent'+FieldCount+'"  class="form-control" style="margin-top: 8px;width: 4%;    margin-left: 21px;"   name="offerAmt[]">&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" id="discount_price'+FieldCount+'"  class="form-control" style="margin-top: 8px;width: 6%;"   name="discTotal[]">&nbsp;&nbsp;&nbsp;&nbsp;<input type="test" id="finaltotal'+FieldCount+'" class="form-control" style="margin-top: 8px;width: 7%;    margin-left: 24px;"  name="ftotalval[]" readonly ><input type="hidden" class="form-control" name="insert-id" value="insert"><input type="hidden" id="offerval'+FieldCount+'" class="form-control" style="margin-top: 8px;width: 6%;"  name="offeridval[]" readonly /><a href="javascript:void(0);" class="remCF" id='+FieldCount+' style="position: absolute;color: red;padding: 5px 35px;font-size: 20px;"><i class="fa fa-trash-o" aria-hidden="true"></i></a></div>'); //add input box

                  }
                  $(wrapper).insertAfter('#create');
                  // }
                  $('#pname'+FieldCount).val(proName);
                  $('#unitpric'+FieldCount).val(response1[0].price);
                  $('#taxNameCgst'+FieldCount).val(response1[0].taxPercent);
                  $('#taxnameSgst'+FieldCount).val(response1[0].taxPercents);
    ...
    function accordingQtyCalulate(str) {
        ...
        if(amtOffer22 > 0 ) {
            ...
            $('#stotal' + res).val(amttot);
            $('#totlgstcgs' + res).val(gstamtTotal23);
            $('#taxAmtCgst' + res).val(cgstp);
            $('#taxAmtSgst' + res).val(gsgstper);
            $('#totalDisc' + res).val(gstamtTotal2);
            $('#taxtotal' + res).val(gstamtTotal.toFixed(2));
            $('#total' + res).val(amtTotal1.toFixed(2));

            var sutota1 = (storeBal) + (gstamtTotal);
            var sutota12 = (storeBal1) + (gstamtTotal);
            document.getElementById('finaltotal'+ res).value = gstamtTotal.toFixed(2);
            ...
            $('#taxAmtCgst' + res).val(cgstp);
            $('#taxAmtSgst' + res).val(gsgstper);
            $('#totlgstcgs' + res).val(gstamtTotal);
            $('#total' + res).val(amtTotal1.toFixed(2));
            $('#taxtotal' + res).val(gstamtTotal.toFixed(2));
            $('#totalDisc' + res).val(0);
            var sutota1 = (storeBal) + (gstamtTotal);
        ...
        $('#update-id').val('update');
    }  function accordingGSTCalulate(str) {
