...
if(isset($_POST['submit_val'])) {
    ...
    //echo "<script>alert('$txt_offerval')</script>";
    ...
    $tuimewise = date('Y');
    //echo "<script>alert('$toword');</script>";
    ...
    $query="INSERT INTO `tbltransaction`( `billno`, `tdate`, `btotal`, atotal,`discount`, `remaining`, `shopid`, `fid`, `modeOfPayment`,custid,remark,statusQT,paid,stateCode,inwords,typeTrnj) VALUES ('$billNo','$todate','$txt_samt1','$txt_gamt','$txt_discount','$remaining','$shopidval','$fid','$modeOfPayment','$cid','$onlineCashCheque','T','$txt_paiding','$txt_stCode','$toword','withTax')";
    $record=mysqli_query($conn,$query);
    $last_row =  mysqli_insert_id($conn);
    foreach ($qty as $key => $valuein) {
        $query="INSERT INTO `tblitem`(`pname`,hsncode, `unitprice`, `quantity`, `discount_price`,`total`, `finaltotal`,`taxtotal`, `taxname`,`taxAmtSgst`, `taxAmtCgst`, `taxNameCgst`, `billno`, `shopid`, `fid`, `tid`, `statusQT`,oid)
 VALUES ('$addProName[$key]','$addHsncode[$key]','$unitPrice[$key]','$qty[$key]','$discTotal[$key]','$totalAmt[$key]','$finalTotalAmt[$key]','$totlgstcgst[$key]','$sgst[$key]','$sgstTotal[$key]','$cgsttotal[$key]','$cgst[$key]','$billNo','$shopidval','$fid','$last_row','T','$txt_offerval[$key]')";
        $record=mysqli_query($conn,$query);
    ...
    if($record) {
        ...
        echo "<script>window.location.href='billInvoiceGeneratedWithtax.php?custid=$cdid&proId=$pid;'</script>";
...
<legend name="legend" id="legend" style="width: 102%;min-height: 70%;    height: auto;/*MARGIN: 0PX -162PX;*/">
...
'>";
        ...
        <form action="" method="post">
            <div id="company">
                ...
                <div>
                    <span>Email-Id:&nbsp;&nbsp;&nbsp;&nbsp;<input type="email" name="email" id="email" style='' ></span>
                </div><br/>
                <div>
    ...
    border-top: 1px solid #6bb16b;">
          <span><h3>
                  <b style="margin-left:0%">HSN-CODE</b>
                  <b style="margin-left:-1%">Item Name</b>
                  <b style="margin-left: 2%">Qty</b>
                  <b style="margin-left:0%">Rate</b>
                  <b style="margin-left: -2%">Total</b>
                  <b style="margin-left: -1%">C-GST</b>
                  <b style="margin-left: -2%">Amt</b>
                  <b style="margin-left: -2%">S-GST</b>
                  <b style="margin-left:-2%">Amt</b>
                  <b style="margin-left: 0%">Net Amt</b>
                  <b style="margin-left: -2%">Disc %</b>
                  <b style="margin-left: -2%">Disc Amt</b>
                        <b style="margin-left:-4%">F-Total</b></span>
                    </div>
