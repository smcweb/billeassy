...
$numphonenumber=0;
    ...
    $query11 = "UPDATE `phonebook` SET `subject`='$subject',`name`='$custname',`phonenumber`='$custnumber',`email`='$custEmail',`address`='$custAddr',`states`='$custState',`gst`='$custGSTNo',`statesCode`='$custStCode',`balanceAmt`='0' WHERE `cid`='$cid'";
    ...
    $query="INSERT INTO `tblquotation`( `custid`, `billno`, `tdate`, `btotal`, `discount`, `atotal`, `inwords`, `shopid`, `fid`, `statesCode`, `modeOfPayment`, `remark`, `statusQT`, `trnjType`) VALUES ('$cid','$billNo','$invoiceDate','$txt_samt1','$txt_disc','$txt_gamt1','$txt_words','$shopidBn','$fid','$custStCode','$txt_modeOfPayment','$txt_cashid','Q','withTax')";
    $record=mysqli_query($conn,$query);
...
VALUES ('$txt_addProName[$key]','$txt_MRP[$key]','$txt_withMRP[$key]','$txt_qty[$key]','$txt_totalAmt[$key]','$txt_totlgstcgst[$key]','$txt_finalTotalAmt[$key]','$txt_discountPrice[$key]','$txt_sgst[$key]','$txt_sgstTotal[$key]','$txt_cgst[$key]','$txt_cgsttotal[$key]','$billNo','$shopidBn','$fid','$txt_offeridval[$key]','$last_row','Q','$txt_addHsncode[$key]')";
        $record = mysqli_query($conn, $query);
    }
    if($record) {
    ...
    setTimeout(function() {
        swal({
            ...
            window.location = 'taxQuation.php?custid=$cdid&proId=$pid';
        });
    }, 500);
</script>";
       /* echo "<script>alert('Wait For Print');</script>";
        echo "<script>window.location.href='taxQuation.php?custid=$cdid&proId=$pid;'</script>";*/
...
<div id="wrapper">
   ...
   /* include_once "header.php";*/ ?>
    ...
    padding: 0px 0px 13px;" placeholder="finance"></td>
                    ...
                    <div class="col-md-3" style="margin: 0px 172px;">
                        <table class="table table-bordered">
                            <thead>
                            <tr>
