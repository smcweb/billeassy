...
if(isset($_POST['submit_val'])) {
    ...
    $query="INSERT INTO `tbltransaction`( `billno`, `tdate`, `btotal`, atotal,`discount`, `remaining`, `shopid`, `fid`, `modeOfPayment`,custid,remark,statusQT,paid,inwords,typeTrnj) VALUES ('$billNo','$todate','$txt_samt1','$txt_gamt','$txt_discount','$remaining','$shopidval','$fid','$modeOfPayment','$cid','$onlineCashCheque','T','$txt_paiding','$gwords','withoutTax')";
    $record=mysqli_query($conn,$query);
...
VALUES ('$addProName[$key]','$addHsncode[$key]','$unitPrice[$key]','$qty[$key]','$totalAmt[$key]','$taxPer[$key]','$finalTotalAmt[$key]','$gstName[$key]','$billNo','$shopidval','$fid','$txt_offerval[$key]','$last_row','$txt_discountPrice[$key]','T')";
        $record=mysqli_query($conn,$query);
